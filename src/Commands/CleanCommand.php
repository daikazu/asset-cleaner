...
{
    public $signature = 'asset-cleaner:clean
    ...
    public $description = 'Delete unused image assets based on the manifest';
    ...
    private function handleManifestMode(AssetCleaner $cleaner, bool $dryRun, bool $force): int
    {
        if (! $cleaner->manifest()->exists()) {
            warning('No manifest file found.');
            note('Run `php artisan asset-cleaner:scan` first to generate a manifest.');

            return self::FAILURE;
        }

        $manifest = $cleaner->manifest()->load();
        $assets = $cleaner->manifest()->getAssets();

        if ($assets->isEmpty()) {
            info('No assets in manifest to delete.');
            $cleaner->manifest()->delete();

            return self::SUCCESS;
        }

        info($dryRun ? 'DRY RUN - No files will be deleted' : 'Preparing to delete assets...');

        note("Found {$assets->count()} assets in manifest ({$manifest['total_size_human']})");

        if (! $dryRun && ! $force) {
            $confirmed = confirm(
                label: "Are you sure you want to delete {$assets->count()} files?",
                default: false,
            );
    ...
    private function handleTrustMode(AssetCleaner $cleaner, bool $dryRun, bool $force): int
    {
        warning('TRUST MODE: Scanning and deleting in one step.');

        if (! $dryRun && ! $force) {
            $confirmed = confirm(
                label: 'This will delete all detected unused assets without review. Continue?',
                default: false,
            );
        ...
        info($dryRun ? 'DRY RUN - Scanning for unused assets...' : 'Scanning and deleting unused assets...');

        $result = $cleaner->cleanAll($dryRun);

        note("Scanned {$result['scanned']} total image assets.");
